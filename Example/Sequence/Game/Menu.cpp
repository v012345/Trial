...
namespace Sequence {
    namespace Game {
        ...
        Menu::Menu() : mImage(0) { mImage = new Image(CMAKE_CURRENT_SOURCE_DIR "data/image/menu.dds"); }

        Menu::~Menu() { SAFE_DELETE(mImage); }
        ...
        void Menu::update(Parent* parent) {
            ...
            int inputNumber = 0;
            for (int i = 0; i < 5; ++i) {
                if (Framework::instance().isKeyTriggered(numberChars[i])) { inputNumber = i; }
            }
            switch (inputNumber) {
                case 1: // 重来
                    parent->state()->reset();

...
#else
#define lfs_mkdir(path) (mkdir((path), S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IXOTH))
#endif
...
int lfs_win32_lstat(const char* path, STAT_STRUCT* buffer) {
    ...
    if (GetFileAttributesEx(path, GetFileExInfoStandard, &win32buffer)) {
        if (!(win32buffer.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT)) { return STAT_FUNC(path, buffer); }
...
static int lfs_lock_dir(lua_State* L) {
    ...
    strcpy(ln, path);
    strcat(ln, lockfile);
    fd = CreateFile(ln, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_DELETE_ON_CLOSE, NULL);
    free(ln);
    if (fd == INVALID_HANDLE_VALUE) { return lfs_win32_pusherror(L); }
...
static int lfs_f_setmode(lua_State* L) { return lfs_g_setmode(L, check_file(L, 1, "setmode"), 2); }
...
static int make_link(lua_State* L) {
    ...
    int is_dir = 0;
    if (STAT_FUNC(oldpath, &oldpathinfo) == 0) { is_dir = S_ISDIR(oldpathinfo.st_mode) != 0; }
    if (!symbolic && is_dir) {
        lua_pushnil(L);
    ...
    int result = symbolic ? CreateSymbolicLink(newpath, oldpath, is_dir) : CreateHardLink(newpath, oldpath, NULL);

    if (result) {
        return pushresult(L, result, NULL);
    } else {
        lua_pushnil(L);
        lua_pushstring(L, symbolic ? "make_link CreateSymbolicLink() failed" : "make_link CreateHardLink() failed");
        return 2;
    }
#endif
}
...
#ifdef _WIN32
    if (!d->closed && d->hFile) { _findclose(d->hFile); }
#else
    if (!d->closed && d->dir) { closedir(d->dir); }
#endif
    d->closed = 1;
    return 0;
}
...
#else
    d->dir = opendir(path);
    if (d->dir == NULL) luaL_error(L, "cannot open %s: %s", path, strerror(errno));
#endif
...
static void push_st_dev(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_dev); }

/* inode's number */
static void push_st_ino(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_ino); }

/* number of hard links to the file */
static void push_st_nlink(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_nlink); }

/* user-id of owner */
static void push_st_uid(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_uid); }

/* group-id of owner */
static void push_st_gid(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_gid); }

/* device type, for special file inode */
static void push_st_rdev(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_rdev); }

/* time of last access */
static void push_st_atime(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_atime); }

/* time of last data modification */
static void push_st_mtime(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_mtime); }

/* time of last file status change */
static void push_st_ctime(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_ctime); }

/* file size, in bytes */
static void push_st_size(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_size); }
...
static void push_st_blksize(lua_State* L, STAT_STRUCT* info) { lua_pushinteger(L, (lua_Integer)info->st_blksize); }
#endif
...
*/
static int file_info(lua_State* L) { return _file_info_(L, STAT_FUNC); }
...
static int push_link_target(lua_State* L) {
    const char* file = luaL_checkstring(L, 1);
#ifdef _WIN32
    HANDLE h = CreateFile(file, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (h == INVALID_HANDLE_VALUE) { return lfs_win32_pusherror(L); }
#endif
    char* target = NULL;
...
static void set_info(lua_State* L) {
    lua_pushliteral(
        L, "LuaFileSystem is a Lua library developed to complement "

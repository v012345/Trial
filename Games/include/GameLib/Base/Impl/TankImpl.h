...
template <class T> inline Tank<T>::Tank(int blockSize) : mBlockSize(blockSize), mSize(0), mLastBlock(&mHead), mLastPos(blockSize), mCurrentBlock(&mHead), mCurrentPos(blockSize) {
    mHead.mNext = &mHead;
    mHead.mElements = 0;
}

template <class T> inline Tank<T>::~Tank() { clear(); }
...
template <class T> inline int Tank<T>::size() const { return mSize; }
...
template <class T> inline void Tank<T>::clear() {
    ...
    while (n != &mHead) {
        ...
        for (int i = 0; i < destructN; ++i) { n->mElements[i].~T(); }
...
template <class T> inline void Tank<T>::add(const T& v) {
    ...
    if (mLastPos == mBlockSize) {
        ...
        if (mLastBlock == &mHead) { // 在第一个节点上设置迭代器
...
template <class T> inline bool Tank<T>::isEnd() const { return (mCurrentBlock == &mHead); }

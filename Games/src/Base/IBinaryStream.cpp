...
IBinaryStream::IBinaryStream(const RefString& s) : mBuffer(s.get()), mSize(s.size()), mPosition(0) {}
...
int IBinaryStream::read(char* dst, int size) {
    if (size > mSize - mPosition) { size = mSize - mPosition; }
    for (int i = 0; i < size; ++i) { dst[i] = mBuffer[mPosition + i]; }
    mPosition += size;
    return size;
}
...
bool IBinaryStream::isEnd() const { return (mPosition >= mSize); }

int IBinaryStream::position() const { return mPosition; }

int IBinaryStream::size() const { return mSize; }

void IBinaryStream::setPosition(int p) { mPosition = p; }

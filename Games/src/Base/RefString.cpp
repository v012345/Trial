...
const char* RefString::get() const { return mString; }

int RefString::size() const { return mSize; }

void RefString::setSize(int size) { mSize = size; }

void RefString::clamp(int first, int last) {
    ASSERT(first < mSize);
    ASSERT(last < mSize && first < last);
    if (first < 0) { first = 0; }
    if (last < 0) { last = mSize - 1; }
    mString = mString + first;
    mSize = last - first + 1; // 需要+1。因为也包含了last
}
...
bool RefString::operator==(const RefString& s) const {
    if (mSize != s.mSize) { return false; }
    for (int i = 0; i < mSize; ++i) {
        if (mString[i] != s.mString[i]) { return false; }
    }
    return true;
}
...
bool RefString::operator==(const char* s) const {
    int i = 0;
    while ((i < mSize) && s[i]) {
        if (mString[i] != s[i]) { return false; }
        ++i;
    }
    return ((s[i] == '\0') && (i == mSize)); // 如果同时完成则为真
}
/*
...
*/
bool RefString::operator!=(const RefString& s) const { return !operator==(s); }

bool RefString::operator!=(const char* s) const { return !operator==(s); }
...
char* RefString::copyOriginal() const {
    ...
    for (int i = 0; i < mSize; ++i) { r[i] = mString[i]; }
    r[mSize] = '\0';
    return r;
}

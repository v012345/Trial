...
class OStringStream::Impl {
  public:
    ...
    void add(double a) {
        ...
        ASSERT(c <= 30);
        for (int i = 0; i < c; ++i) { mBuffer.add(s[i]); }
    }
    void add(float a) { // 现在该怎么做。
        char s[16];
        int c = toString(s, a, mPrecision);
        ASSERT(c <= 16);
        for (int i = 0; i < c; ++i) { mBuffer.add(s[i]); }
    ...
    void add(char a) {
        if ((a == '\n') || (a == '\t')) {
            mBuffer.add(a); // 不转换（空白字符）
        } else if (a >= 0x20 && a <= 0x7e) {
            mBuffer.add(a); // 不转换
        } else {
            mBuffer.add('%');
            char s[2];
            int c = toString16(s, a); // 16进制
            ASSERT(c <= 2);
            for (int i = 0; i < c; ++i) { mBuffer.add(s[i]); }
        }
    }
    ...
    void add(const string& a) { add(a.c_str()); }
    void add(const char* s, int n) {
        for (int i = 0; i < n; ++i) { mBuffer.add(s[i]); }
    }
    void get(Array<char>* a) const {
        a->setSize(mBuffer.size() + 1);
        if (mBuffer.size() > 0) { mBuffer.copyTo(&(*a)[0]); }
        (*a)[mBuffer.size()] = '\0'; // NULL终止
    }
...
OStringStream::~OStringStream() { SAFE_DELETE(mImpl); }
...
void OStringStream::write(const char* s, int size) { mImpl->add(s, size); }
...
OStringStream& OStringStream::operator<<(OStringStream& (*f)(OStringStream&)) { return (*f)(*this); }

void OStringStream::get(Array<char>* a) const { mImpl->get(a); }

void OStringStream::get(string* a) const {
    if (mImpl->size() > 0) {
        Array<char> t;
        mImpl->get(&t);
        *a = string(&t[0], t.size() - 1); // 由于存在许多NULL终止，因此将减去ArraySize
    }
}

int OStringStream::size() const { return mImpl->size(); }

void OStringStream::clear() { mImpl->clear(); }

OStringStream& endl(OStringStream& a) { return a.endl(); }

OStringStream& hex(OStringStream& a) { return a.hex(); }

OStringStream& dec(OStringStream& a) { return a.dec(); }

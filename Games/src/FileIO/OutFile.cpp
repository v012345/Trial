...
OutFile OutFile::create(const char* filename, const char* data, int size) {
    OutFile r;
...
void OutFile::release() {
    if (mImpl) {
        mImpl->release();
        if (mImpl->referenceCount() == 0) { gManagerImpl->destroyOutFile(mImpl); }
    }
    mImpl = 0;
}
...
bool OutFile::operator==(const OutFile& o) const { return (mImpl == o.mImpl); }

bool OutFile::operator!=(const OutFile& o) const { return (mImpl != o.mImpl); }

OutFile::operator void*() const { return (mImpl == 0) ? 0 : reinterpret_cast<void*>(~0); }

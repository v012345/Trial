...
class Semaphore::Impl : public ReferenceType {
  public:
    Impl(int startCount, int maxCount) : mMaxCount(maxCount) { mHandle = CreateSemaphore(NULL, startCount, mMaxCount, NULL); }
    ~Impl() { CloseHandle(mHandle); }
    void decrease() { WaitForSingleObject(mHandle, INFINITE); }
...
Semaphore Semaphore::create(int startCount, int maxCount) {
    ...
    if (maxCount < startCount) { maxCount = startCount; }
    Semaphore r;
    r.mImpl = NEW Impl(startCount, maxCount);
    return r;
}

void Semaphore::decrease() { mImpl->decrease(); }

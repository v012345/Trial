...
void DebugStream::begin() { mImpl = NEW Impl; }

void DebugStream::end() { SAFE_DELETE(mImpl); }
...
DebugStream& DebugStream::endl() {
    ...
    Array<char> a;
    mImpl->mBuffer.get(&a);
    OutputDebugStringA(&a[0]); // 直接调用ASCII版本
    mImpl->mBuffer.clear();
    mImpl->mMutex.unlock();
    return *this;
}
...
DebugStream& endl(DebugStream& a) { return a.endl(); }

DebugStream& hex(DebugStream& a) { return a.hex(); }

DebugStream& dec(DebugStream& a) { return a.dec(); }

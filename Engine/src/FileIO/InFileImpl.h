...
class InFile::Impl : public ReferenceType {
  public:
    ...
    Impl() : mData(0), mSize(0), mIsError(false), mIsFinished(false) {}
    ~Impl() {
    ...
    void setError() { // 如果有错误就结束
        mIsError = mIsFinished = true;
    }
    void setIterator(It it) { mIterator = it; }
    It iterator() const { return mIterator; }
    void set(char* data, int size) {
        ASSERT(!mIsFinished);
        mData = data;
        mSize = size;
        if (!mData) { mIsError = true; }
        mIsFinished = true;
    }

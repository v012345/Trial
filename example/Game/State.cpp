...
State::State(int stageID) : mImage(0), mDynamicObjects(0), mDynamicObjectNumber(0), mStageID(stageID) {
...
void State::draw() const {
    // 绘制背景
    for (int y = 0; y < HEIGHT; ++y) {
        for (int x = 0; x < WIDTH; ++x) { mStaticObjects(x, y).draw(x, y, mImage); }
    }
    // 绘制前景
    for (int i = 0; i < mDynamicObjectNumber; ++i) { mDynamicObjects[i].draw(mImage); }
    // 绘制冲击波
    for (int y = 0; y < HEIGHT; ++y) {
        for (int x = 0; x < WIDTH; ++x) { mStaticObjects(x, y).drawExplosion(x, y, mImage); }
    }
}
...
void State::update() {
    ...
    for (int y = 0; y < HEIGHT; ++y) {
        for (int x = 0; x < WIDTH; ++x) {
            if (mStaticObjects(x, y).checkFlag(StaticObject::FLAG_EXPLODING)) { setFire(x, y); }
        }
    }
    ...
    for (int y = 0; y < HEIGHT; ++y) {
        for (int x = 0; x < WIDTH; ++x) {
            ...
            if (o.checkFlag(StaticObject::FLAG_BOMB)) { ++bombNumber[o.mBombOwner->mPlayerID]; }
        }
    }
    // 与动态对象循环
    for (int i = 0; i < mDynamicObjectNumber; ++i) {
        ...
        for (int j = 0; j < 2; ++j) {
            if (o.mLastBombX[j] >= 0) { // 0以上的话里面有什么。
                if (!o.isIntersectWall(o.mLastBombX[j], o.mLastBombY[j])) { o.mLastBombX[j] = o.mLastBombY[j] = -1; }
            }
        }
        // 获取当前单元格
        int x, y;
    ...
    // 下一个 判断敌人与玩家之间的碰撞。
    for (int i = 0; i < mDynamicObjectNumber; ++i) {
        for (int j = i + 1; j < mDynamicObjectNumber; ++j) { mDynamicObjects[i].doCollisionReactionToDynamic(&mDynamicObjects[j]); }
    }
}
...
bool State::hasCleared() const {
    // 清除是否没有敌人
    for (int i = 0; i < mDynamicObjectNumber; ++i) {
        if (mDynamicObjects[i].isEnemy()) { return false; }
    }
    return true;
}
...
bool State::isAlive(int playerID) const {
    ...
    for (int i = 0; i < mDynamicObjectNumber; ++i) {
        if (mDynamicObjects[i].mType == DynamicObject::TYPE_PLAYER) {
            if (mDynamicObjects[i].mPlayerID == playerID) { return true; }
        }
    }
    return false;
}

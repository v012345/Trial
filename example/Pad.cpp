...
bool Pad::isOn(Button b, int pid) {
    ...
    if (im.joystickNumber() > pid) {
        ...
        int table[] = {
            0, 1, Input::Joystick::BUTTON_UP, Input::Joystick::BUTTON_DOWN, Input::Joystick::BUTTON_LEFT, Input::Joystick::BUTTON_RIGHT, 2,
        };
        if (j.isOn(table[b])) { ret = true; }
    }
    return ret;
}
...
bool Pad::isTriggered(Button b, int pid) {
    ...
    if (im.joystickNumber() > pid) {
        ...
        if (j.isTriggered(table[b])) { ret = true; }
    }
    return ret;
}
